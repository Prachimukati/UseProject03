    ...
    if (isNaN(currentValue) || /[^0-9]/.test(currentValue)) {
        ...
        if (currentValue.length > maxLength) {
            ...
            if (currentValue.length > 0 && currentValue.charAt(0) <= '5') {
                errorMessage.textContent = 'Phone number must start with a digit greater than 5.';
                inputElement.value = '';
            } else {
                errorMessage.textContent = '';
            }
...
function validateDateInput(inputElement, errorElementId, minDate, maxDate) {
    const errorMessage = document.getElementById(errorElementId);
    const currentValue = inputElement.value;
    ...
    const enteredDate = new Date(`${dateParts[0]}-${dateParts[1]}-${dateParts[2]}`);

    // Validate date range if minDate or maxDate is provided
    if (minDate && enteredDate < new Date(minDate)) {
        errorMessage.textContent = `Date cannot be earlier than ${minDate}.`;
        return false;
    }

    if (maxDate && enteredDate > new Date(maxDate)) {
        errorMessage.textContent = `Date cannot be later than ${maxDate}.`;
        return false;
    }

    // If no errors, clear error message and return true
    errorMessage.textContent = '';
    return true;
}